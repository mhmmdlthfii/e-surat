...
export default function SuratKeluar({ currentUserId, currentUserRole, currentUserName, triggerToast, openPrintPreview }: SuratKeluarProps) {
  ...
  return (
    <div id="suratkeluar_root" className="space-y-6">
      ...
      <div className="glass rounded-2xl p-4 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-white dark:bg-zinc-950 pl-10 pr-4 py-2 border border-zinc-200 dark:border-zinc-850 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            />
          </div>

          <div className="flex flex-wrap items-center gap-2">
            {/* Filter Category */}
            <div className="flex items-center space-x-1 border border-zinc-200 dark:border-zinc-850 rounded-xl px-2.5 py-1 bg-white dark:bg-zinc-950 text-xs">
              <Filter size={14} className="text-zinc-400" />
              <select 
                id="select_keluar_category"
                value={filterCategory} 
                onChange={(e) => setFilterCategory(e.target.value)}
                className="bg-transparent border-none text-zinc-700 dark:text-zinc-300 focus:outline-none font-medium cursor-pointer py-1"
              >
                {categories.map((c, idx) => (
                  <option key={idx} value={c} className="bg-white dark:bg-zinc-900">{c === 'Semua' ? 'Semua Kategori' : c}</option>
                ))}
              </select>
            </div>
            ...
            <div className="flex items-center space-x-1 border border-zinc-200 dark:border-zinc-850 rounded-xl px-2.5 py-1 bg-white dark:bg-zinc-950 text-xs">
              ...
              >
                {statusOptions.map((s, idx) => (
                  <option key={idx} value={s} className="bg-white dark:bg-zinc-900">{s === 'Semua' ? 'Semua Status' : s}</option>
                ))}
              </select>
            </div>
          </div>
